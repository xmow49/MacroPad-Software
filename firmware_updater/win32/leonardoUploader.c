...
int checkNewPorts(int oldPort[], int newPort[], int output[], int size);


// Port list
int ports[20]; // Array of PC ports 20 is a Big number of Serial ports
int portsAfterReset[20];
...
int main(int argc, char *argv[])
{
  ...
  dcbSerialParams.BaudRate = CBR_2400;   // Setting BaudRate = 2400
  ...
  updatePortList(portsAfterReset);

  int newPort[20];
  checkNewPorts(ports, portsAfterReset, newPort, 20);
  if(newPort[0] > 254)
  {
    printf("  Error No New Port Detected\n");
    return 6;
  }
...
void makeAvrdudeCommand(int Port, char *filename)
{
  ...
  sprintf(cmd, "avrdude.exe -pm32u4 -cavr109 -F -D -P COM%d -Uflash:w:%s", Port, filename);
}

// check new ports in the list
int checkNewPorts(int oldPort[], int newPort[], int output[], int size)
{
  int index = 0;
  for (int i = 0; i < size; i++)
  {
    ...
    if (!found)
    {
      output[index] = newPort[i];
      index++;
      printf("New port found: %d\n", newPort[i]);
    }
  }
}